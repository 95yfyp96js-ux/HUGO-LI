...
void main() {
  ...
  Future<String> get(String url, {String? token}) async {
    final res = await app.handler(
      Request(
        'GET',
        Uri.parse(url),
        headers: {'cookie': 'sid=${token ?? fx.bossToken}'},
      ),
    );
    return res.readAsString();
  }

  Future<String> post(String body, {String? token}) async {
    final res = await app.handler(
      Request(
        'POST',
  ...
  test('GET /：一頁同時有左邊收款表單與右邊活盤五格', () async {
    fx.seedLoan();
    final html = await get('http://x/settle');
  ...
  test('選了借款後列出未清期別，金額顯示到分', () async {
    fx.seedLoan();
    final html = await get('http://x/settle?loan_id=${fx.loanId}');
